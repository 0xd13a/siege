...
#define STR_BUF_LENGTH  22 // Bumping it up to accommodate the newline character fgets() reads
...
int check_credentials() {
	...
	if (!fgets(login.username, STR_BUF_LENGTH, stdin)) {
		return 0;
	}
	...
	// VULNERABILITY: password that is 21 characters instead of 20 overwrites login.unknown with zero (zero byte at the end of the string)
	if (!fgets(login.password, STR_BUF_LENGTH, stdin)) {
		return 0;
	}
...
int get_menu_choice(int max_value) {
...
void handle_menu() {
	...
	// VULNERABILITY: negative choice number allows alarm disarming
	int choice;
	...
	do {
		...
		// VULNERABILITY: choice 2 and 3 must only be allowed for admins
		if (choice == 2) {
