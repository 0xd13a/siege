...
class User:
    ...
    def __init__(self, user, monitorOnly=1):
        self.user = user
        self.monitor = monitorOnly
...
def get_user(request):
    ...
    if token is None:
        ...
        return User(decoded_data["user"],
                    monitorOnly=int(decoded_data["monitor"]))
...
def set_current_user(response, user):
    ...
    if user is None:
        ...
        token = jwt.encode(payload={"user": user.get_username(),
                                    "monitor": user.is_monitor()},
                           key=jwt_key, algorithm="HS256")
...
def change():
    ...
    if user.is_monitor():
        return "Monitors are forbidden from making changes.", 403
    ...
    maxSafe = int(NUMBER_OF_CHANNELS_IN_REACTOR * 0.7)
    minReq = int(NUMBER_OF_CHANNELS_IN_REACTOR * 0.2)

    if bundles_to_insert < 0:
        return "Bad Request", 400

    if bundles_to_remove < 0:
        return "Bad Request", 400

    if bundles_to_remove > bundles_candidates_for_removal:
        return "That will waste good fuel", 400

    if (bundles_used + bundles_to_insert - bundles_to_remove) > maxSafe:
        return "That would cause a reactor meltdown", 400

    if (bundles_used + bundles_to_insert - bundles_to_remove) < minReq:
        return "That would cause a reactor shutdown", 400
...
def login():
    ...
    if user.get_username() is None:
        ...
        cur = con.cursor()
        for row in cur.execute("select user, pwd, monitor_only from users"
                               " where user=?", (username,)):
            rows.append(row)
        cur.close()
        con.close()
        ...
        if len(rows) >= 1:
            ...
            if bcrypt.checkpw(pwd.encode("utf-8"), val[1].encode("utf-8")):
                current_user = User(val[0], monitorOnly=val[2])
                resp = make_response(redirect("/status"))
